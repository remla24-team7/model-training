...
def mutate_protocol(parsed_url):
    ...
    return [urlunparse((protocol, parsed_url.netloc, parsed_url.path, parsed_url.params, parsed_url.query, parsed_url.fragment))]
...
def mutate_domain(parsed_url):
    ...
    return [urlunparse((parsed_url.scheme, domain, parsed_url.path, parsed_url.params, parsed_url.query, parsed_url.fragment))]
...
def mutate_path(parsed_url):
    ...
    return [urlunparse((parsed_url.scheme, parsed_url.netloc, path, parsed_url.params, parsed_url.query, parsed_url.fragment)) for path in path_mutants]
...
def mutate_query(parsed_url):
    ...
    return [urlunparse((parsed_url.scheme, parsed_url.netloc, parsed_url.path, parsed_url.params, query, parsed_url.fragment)) for query in query_mutants]
...
def generate_single_feature_mutants(url, num_mutations=5):
    ...
    if num_mutations > len(unique_mutants):
        num_mutations = len(unique_mutants)
...
def generate_mutamorphic_sample(url):
    ...
    preprocessed_urls = Preprocess.preprocess(mutated_urls) #preprocess the mutated URLs
    preprocessed_original = Preprocess.preprocess([url])
...
def generate_mutamorphic_dataset(X_test):
    ...
    mutated_samples = []
    for url in X_test:
        mutated_sample = generate_mutamorphic_sample(url)
